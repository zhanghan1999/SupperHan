...
| **drilldowns** | 下钻目标：**某个值或某个按钮能把用户带到另一个页面**（同应用换页 / 弹层 / 跨端口换应用 / 导出） | `route` → 另一条 screen | 建页面间图，并**递归展开目标页的 actions 与 drilldowns**（§4.6） | ✅（同应用）/ ❌（跨应用，§4.7） | 一期 ❌（见 §9 F-15e） |
...
5. **`drilldowns` 把六段从"一张表"变成"一张图"**：一个页面可以延展多个页面，而被带出的页面自己又是一条
   screen，**它的按钮与下钻同样要学** —— 所以学习必然是图上的一次**闭包展开**，必须带终止闸门（§4.6）。
...
需求原话的要点：**某个值或某个按钮可以把用户带到另一个页面**，一个页面能延展多个页面，并且
**目标页里的按钮、下钻也要学到**。后果：`drilldowns` 不是终端数据，而是**指向另一条 screen 的边**。

必须有四道终止闸门，缺任一条就会跑出无限或失控的产物：

| 闸门 | 判据 | 到线怎么办 |
|---|---|---|
| 去重 | 以**归一化后的 `route`** 为 key，一条 screen 在一个代际里只展开一次 | 再次命中只追加一条入边 `drilledFrom`，**不重抽** actions / dataSources |
| 环 | 展开栈上出现已访问的 key（A 下钻 B、B 回跳 A 很常见） | 记 `cycle: true` 并停该支 |
| 深度 | `maxDepth`（建议默认 3） | 停并标 `truncated: depth` |
| 预算 | `maxScreens`（建议默认 200） | 停并标 `truncated: budget` |

`truncated` 必须写进 `batch` 正文与 `index.md` 计数，**不许静默截断** —— 否则读者会以为手里那份是全集。
上限只能被 L2 收紧，与 `fastPath` 预算被 `HARD_CAPS` 夹取同纪律。
...
drilldowns:    <route> → 另一条 screen（同应用则继续展开；跨应用标 outOfScope）  <模板文件>:<line>
               …… 或停线标注 `truncated: depth|budget` / `cycle: true`
```
...
| 4 | "下钻"具体指哪种 | 说要问用户 | ✅ **已由用户答复**：某个值或某个按钮把用户带到另一个页面（同应用换页 / 跨端口换应用），且一个页面可延展多个页面 —— 见 §4.6 闭包。仍开的只剩一个口径问题：**弹层算不算一条独立 screen** |
...
- **下钻闭包把"一次性遍历"变成图展开**，于是多出三类以前不存在的失效：无限递归（环）、预算爆炸
  （几百页 × 每页多个下钻）、以及**把跨应用跳转误记成缺陷**。四道闸门（§4.6）与 `outOfScope`（§4.7）
  就是为这三样设的。
